...
def get_quote(url: str, network: str, invoice: str, refund: str):
    ...
    return requests.post(url=f"{url}/swaps/", json=data, timeout=60)
...
def check_status(url: str, network: str, invoice: str, redeem_script: str):
    ...
    return requests.post(url=f"{url}/swaps/check", json=data, timeout=60)
...
def broadcast_tx(url: str, network: str, transaction: str):
    ...
    return requests.post(url=f"{url}/transactions/", json=data, timeout=60)
...
def get_exchange_rates(url: str, ):
    ...
    """
    return requests.get(url=f"{url}/exchange_rates/", timeout=60)
...
def get_active_networks(url: str, ):
    ...
    """
    return requests.get(url=f"{url}/networks/", timeout=60)
