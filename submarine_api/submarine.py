...
def get_quote(network: str, invoice: str, refund: str):
    """POST a new swap
    """
    data = {"network": network, "invoice": invoice, "refund": refund}
    return requests.post(url=f"{SUBMARINE_API}/swaps/", json=data, timeout=30)
...
def check_status(network: str, invoice: str, redeem_script: str):
    ...
    return requests.post(url=f"{SUBMARINE_API}/swaps/check", json=data, timeout=30)
...
def broadcast_tx(network: str, transaction: str):
    ...
    return requests.post(url=f"{SUBMARINE_API}/transactions/", json=data, timeout=30)
...
def get_address_details(address: str, network: str):
    ...
    return requests.get(
        url=f"{SUBMARINE_API}/address_details/{network}/{address}",
...
def get_exchange_rates():
    ...
    """
    return requests.get(url=f"{SUBMARINE_API}/exchange_rates/", timeout=30)
...
def get_invoice_details(network: str, invoice: str):
    ...
    return requests.get(
        url=f"{SUBMARINE_API}/invoice_details/{network}/{invoice}",
        params=json.dumps(params),
        timeout=30,
    )
...
def get_active_networks():
    ...
    """
    return requests.get(url=f"{SUBMARINE_API}/networks/", timeout=30)
